...
class DisplayBlogPostsForAnAuthorController extends CommonControllerForClients
{
    /**
    ...
    public function DisplayBlogPostsForAnAuthor($authorNamecalculated)
    {
        ...
        if (!isset($this->messages)) {
            ...
            return view(config('lasallesoftware-frontendapp.lasalle_path_to_front_end_view_path') . '.blog.pages.allblogposts', [
                ...
                'prev_page_url'       => $this->getPageURLForPagination($body->prev_page_url, 'author/'.str_replace(' ', '%20', $authorNamecalculated)),
                'next_page_url'       => $this->getPageURLForPagination($body->next_page_url, 'author/'.str_replace(' ', '%20', $authorNamecalculated)),
