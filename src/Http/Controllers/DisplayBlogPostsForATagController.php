...
class DisplayBlogPostsForATagController extends BaseFrontendController
{
    ...
    public function DisplayBlogPostsForATag($tagTitle)
    {
        $endpointPath = $this->getEndpointPath('alltagblogposts');
        $httpRequest  = 'GET';
        $slug         = $tagTitle; 
        ...
        if (!isset($this->messages)) {
            ...
            return view(config('lasallesoftware-libraryfrontend.lasalle_path_to_front_end_view_path') . '.blog.pages.allblogposts', [
                ...
                'prev_page_url'       => $this->getPageURLForPagination('tag/'.str_replace(' ', '%20', $tagTitle), $body->prev_page_url),
                'next_page_url'       => $this->getPageURLForPagination('tag/'.str_replace(' ', '%20', $tagTitle), $body->next_page_url),
