...
class DisplayBlogPostsForATagController extends CommonControllerForClients
{
    /**
    ...
    public function DisplayBlogPostsForATag($tagTitle)
    {
        ...
        $path = $this->getApiPath('alltagblogposts');

        $response = $this->sendRequestToLasalleBackend($comment, $path, $tagTitle);
        ...
        if (!isset($this->messages)) {
            ...
            return view(config('lasallesoftware-frontendapp.lasalle_path_to_front_end_view_path') . '.blog.pages.allblogposts', [
                ...
                'prev_page_url'       => $this->getPageURLForPagination($body->prev_page_url, 'tag/'.str_replace(' ', '%20', $tagTitle)),
